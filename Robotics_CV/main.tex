...
Systematically deploying and evaluating state-of-the-art robot foundation models, including vision-language-action (VLA) models and world action models:
\begin{itemize}[noitemsep, topsep=2pt]
\item Built a low-cost bimanual manipulator testbed ($<$\$1,200) inspired by Mobile ALOHA with simple human-to-machine interfacing for long-horizon household tasks
\item Deployed and evaluated DreamZero, \(\pi\)0.5, SmolVLA, ACT on pick-and-place and household cleaning tasks across simulation and real-world settings
\end{itemize} \\
...
\begin{itemize}[noitemsep, topsep=2pt]
\item Outperformed baseline decision-making by 38\% on resource delivery metrics in contested logistics scenarios and reduced solution compute time from 24+ hours of manual planning to under 5 minutes
\item Received multiple Air Force SBIR Phase I/II/III contracts and a TACFI selection, totaling seven figures in contract funding
...
Advanced a PhD-stage prototype to secure the first SBIR Phase I and II contracts with Air Force Agility Prime:
\begin{itemize}[noitemsep, topsep=2pt]
\item Developed a rapid multi-agent motion and mission planner with decentralized Monte Carlo Tree Search (MCTS) for autonomous unmanned aerial and ground vehicle planning (20+ vehicles)
\end{itemize} \\

08/2016 -- 12/2022 &
\textbf{Autonomous Systems Group}, UT Austin -- \textbf{Graduate Research Assistant} \newline
Developed novel robotic systems and algorithms via deep learning, formal methods, and controls in collaboration with Caltech, UC Berkeley, Northeastern, and RPI:
\begin{itemize}[noitemsep, topsep=2pt]
...
Built embedded electronics and sensing mechanisms for nano-scale signal detection in Electrohydrodynamic Jet (e-jet) printing in Prof. Kira Barton's lab. \\
...
& \textbf{Robotics \& Control:} Bimanual Manipulation, Kinematics (forward, inverse), Whole-Body Control, Reinforcement Learning (PPO, QMIX), Deep Learning, Perception (SLAM, VIO, OpenCV), Tactile Sensing, Motion Planning (trajectory generation, trajectory optimization), State Estimation (Kalman Filters, EKF, EKF2), Feedback Control (PID, Optimal Control, Nonlinear Control), Model Predictive Control (MPC) \\

& \textbf{VLA \& Foundation Models:} DreamZero, \(\pi\)0.5, SmolVLA, ACT, Diffusion Policies, Imitation Learning (DAgger, teacher-student learning), Sim-to-Real Transfer \\

& \textbf{Large Language Models:} Llama 3/4, Qwen (local deployment); OpenAI GPT-4o/5, Cohere Command A, Claude (cloud/API) \\

& \textbf{Programming \& Tools:} Python, C++, PyTorch, ROS2, Isaac Sim/Isaac Lab (domain randomization, sim-to-real), Embedded Systems, Docker, Linux, Edge Deployment, Systems Design, CAD (SolidWorks, OnShape), Git \\
...
\begin{SectionTable}{\headingfont Service}
